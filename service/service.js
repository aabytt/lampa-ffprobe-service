...
service.register("ffprobe", function(message) {
	...
	dns_resolve = dns(parsedUri.hostname, function onLookup(err, addresses, family) {
		...
		child = exec('./ffprobe -v error -print_format json -show_entries stream=index,codec_type,codec_name,codec_long_name,codec_time_base,sample_rate,channels,channel_layout,bit_rate:stream_tags=title,handler_name,language,BPS ' + '"' + resolvedUri+'"',
